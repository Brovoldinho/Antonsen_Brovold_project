...
```
Calculating the linear and non linear regression between stock value and price each kg NOK. As we can see the p-value < significant level, which means that the stock value and price is positiv related. So when the price for salmon increases or decreases the stock value for this firms will have the same effect. This variables are stronlgy related.
...
```
Calculating the linear and non linear regression between value and volume aswell. This is also positiv related, meaning that the p-value < significant level. So when the stock value for this firms increases or decreases so will the volume of salmon follow. This variables are stronlgy related.
...
```
Using the same method we can also calculate the optimal volume this firms should be selling. The optimal volume is 19636,21 ton, we can also calculate the optimal level which is 17446.05 tons and 21826.38 using 0,05 correlation level.
